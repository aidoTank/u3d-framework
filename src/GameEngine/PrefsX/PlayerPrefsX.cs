...
namespace GameEngine
{
    public class PlayerPrefsX
    {
        ...
        public static bool SetBytes(String key, byte[] bytes)
        {
            return SaveBytes(key, bytes);
        }
        ...
        public static byte[] GetBytes(String key)
        {
            ...
            try {
                char[] cArray = PlayerPrefs.GetString(key).ToCharArray();
                bytes = System.Convert.FromBase64CharArray(cArray, 0, cArray.Length);
            } catch {
                return null;
            }
            return bytes;
